...
SAMPLE_SIZE = 2000
RANDOM_SEED = 45
OUTPUT_PATH = "data_aug/tweets_keyword_engagement_2000.parquet"
...
ENGAGEMENT_PROPORTIONS = {
    'low':    0.50,   # 40%  (reflects majority of organic discourse)
    'medium': 0.30,   # 30%
    'high':   0.20,   # 30% (oversampled relative to pool for diversity)
}
...
REPUBLICAN_KEYWORDS = {
    'hashtags': [
        ...
        # Additional common Republican hashtags (need source)
...
DEMOCRAT_KEYWORDS = {
    'hashtags': [
        ...
        # Additional common Democrat hashtags (need source)
...
# checked if tweet has hashtags, handling various formats (None, list, string)
# Returns True if there are hashtags, False if None, empty, or no hashtags
...
# Classify tweets into republican/democrat/neutral based on keyword matching in hashtags and text.
# This is a heuristic approach to capture political subject matter, not author ideology.
# Hashtags are given priority as they are stronger signals of political content, but text is also checked as a fallback.
# Returns 'republican' if more Republican keywords are found, 'democrat' if more Democrat keywords are found, or 'neutral' if neither or equal.
...
# Derive post type based on retweetedTweet, quotedTweet, and in_reply_to_screen_name fields.
# Args: row: Dictionary or DataFrame row containing tweet data
# Returns: str: One of 'retweet', 'quote', 'reply', or 'tweet'
...
def load_august_tweets():
    ...
    """
    print("STREAMING AUGUST 2024 DATA")
    print("-" * 70)
...
def balanced_sample(df, n):
    ...
    result = result.drop(columns=['has_hashtags'])
...
# Args: tweets_list (list): List of tweet dictionaries
# Returns: pd.DataFrame: Processed sample, or None if insufficient data
...
def main():
    print()
    print("-" * 70)
    print("SAMPLING POLITICAL TWEET DATASET SAMPLE FROM AUGUST 2024")
    print("-" * 70)
